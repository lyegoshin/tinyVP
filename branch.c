 ...
 * Copyright (C) 2017 by Leonid Yegoshin
 */
...
int __MIPS16e_compute_return_epc(struct pt_regs *regs)
{
	...
	case MIPS16e_rr_op:
		if (inst.rr.func == MIPS16e_jr_func) {
			...
			if (inst.rr.ra)
				regs->cp0_epc = regs->gpr[31];
			else
				regs->cp0_epc =
				    regs->gpr[reg16to32[inst.rr.rx]];
 ...
 */
int __compute_return_epc_for_insn(struct exception_frame *regs,
				  unsigned long instr)
{
	unsigned int bit;
	...
	case spec_op:
		...
		case jalr_op:
			gpr_write(regs, insn.r_format.rd, epc + 8);
			regs->cp0_epc = gpr_read(regs, insn.r_format.rs);
			break;
		case jr_op:
#ifdef CONFIG_CPU_MIPSR6
			if (!mipsr2_emulation) {
...
#endif
		case bltz_op:
			if ((long)gpr_read(regs, insn.i_format.rs) < 0) {
	 ...
	 */
	case jal_op:
		gpr_write(regs, 31, regs->cp0_epc + 8);
	case j_op:
		epc += 4;
		epc >>= 28;
		epc <<= 28;
...
//                       set_isa16_mode(regs->cp0_epc);
			panic_thread(regs, "JALX detected\n");
		break;
...
#endif
	case beq_op:
		if (gpr_read(regs, insn.i_format.rs) ==
		    gpr_read(regs, insn.i_format.rt)) {
			epc = epc + 4 + (insn.i_format.simmediate << 2);
	...
	case jump_op:
		if (insn.i_format.rs)   /* beqzc */
			...
			epc = gpr_read(regs, insn.i_format.rt) + insn.i_format.simmediate;
		regs->cp0_epc = epc;
		break;
	...
	case lwc2_op: /* This is bbit0 on Octeon */
		if ((regs->gpr[insn.i_format.rs] & (1ull<<insn.i_format.rt))
		     == 0)
			epc = epc + 4 + (insn.i_format.simmediate << 2);
		else
			epc += 8;
...
#ifndef CONFIG_CPU_MIPSR6
sigill:
	panic_thread(regs, "DSP branch but not DSP ASE - panic thread\n");

	return -EFAULT;
#endif
}

int __compute_return_epc(struct exception_frame *exfr)
{
	unsigned long instr;
	...
	instr = exfr->cp0_badinstp;
	return __compute_return_epc_for_insn(exfr, instr);

unaligned:
	panic_thread(exfr, "Unaligned epc - panic thread.\n");
	return -EFAULT;
